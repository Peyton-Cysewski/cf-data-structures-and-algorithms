...
const isNum = (input) => {
  // return /\d/.test(input);
  return Boolean(input.toString().match(/\d/)); // lol
};
...
const isCapitalized = (str) => {
  return str.match(/\b[A-Z]+[a-z]*\b/g) || []; // credit to Joe Pennock

  // let arr = [];
  // let search = str.match(/\b[A-Z]+[a-z]*\b/g);
  // if (search) {
  //   return search;
  // }
  // else {
...
const findShells = (str) => {
  return str.match(/\bs((h)?(ea)?(eash)?)?(ells)\b/gi)
};
