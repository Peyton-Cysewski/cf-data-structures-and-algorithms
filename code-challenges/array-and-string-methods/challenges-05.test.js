...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    result.push(ingredient.slice(ingredient.indexOf(ingredient.match(/\b[\w-]+/g)[2])));
  });
  return result;
};
...
const extractVowels = (str) => {
  let arr = [];
  let str2 = str;
  arr.push(str.replace(/[aeiou]/g, ''));
  arr.push(str2
    .replace(/[^aeiouAEIOU]/g, '') // it is a string with only the vowels
    .toLowerCase() // makes all the characters lower case
    .split('') // splits each character and it becomes a new array of the characters
    .sort( (a,b) => a.charCodeAt(0) - b.charCodeAt(0)) // sorts the array alphabetically
    .join() // puts the characters back together
    .replace(/[^aeiou]/gi, '')); // replaces all empty spaces and commas with nothing
  return arr;
};
