...
const onlyOddChars = (str) => {
  return str.split('').reduce((acc,cur,idx) => acc = (idx % 2 === 1) ? acc + cur : acc,'');
};
...
const unenrollBrook = (arr) => {
  return arr.map(item => item.filter(string => !string.includes('Brook')));
};
...
const sortByDay = (arr) => {
  ...
  for (let i = 0; i < daysOfWeek.length; i++) {
    result.push(arr.filter(string => string.includes(daysOfWeek[i])));
  }
  return result;
};
