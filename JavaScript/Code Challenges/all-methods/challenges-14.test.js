...
const toTitleCase = (arr) => {
  return arr.map(str => str.replace(/^\w/, str.charAt(0).toUpperCase()));
};
...
let biggerThanLuke = (arr) => {
  return arr.filter((char, idx, array) => (parseInt(char.mass) > parseInt(array[0].mass)) ? true : false).map(item => item.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  let type = (typeof arr[property] === 'number') ? true : false;
  return arr.sort((a,b) => {
    if (type) {
