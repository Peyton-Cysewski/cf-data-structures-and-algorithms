...
const reversedString = (str) => {
  ...
  return charArr.reduce( (acc, cur) => cur + acc, '')
};
...
// Inspired by the Joes and their quest for all things one-lined
const extractChildren = arr => arr.filter(char => char.name.includes('a')).reduce((acc, cur) => (cur.children ? acc = acc.concat(cur.children) : acc), []);
