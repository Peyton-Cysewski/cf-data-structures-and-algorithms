...
const hasChildrenEntries = (arr, character) => {
  let result = false;
  arr.forEach(obj => {
    if (character === Object.entries(obj)[0][1]) {
      result = Boolean(Object.entries(obj)[2][1].length);
    }
  });
  return result;
};
...
const houseSurvivors = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let alive = true;
    let hadSpouse = true;
    for (let k = 0; k < deceasedSpouses.length; k++) {
      if (arr[i].spouse === deceasedSpouses[k]) {
        alive = false;
      }
